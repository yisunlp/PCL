    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1 = forward_and_adapt_eata(x, self.model,
                                                                                            self.optimizer,
...
def softmax_entropy(x: torch.Tensor) -> torch.Tensor:
    ...
    temprature = 1
    x = x / temprature
    x = -(x.softmax(-1) * x.log_softmax(-1)).sum(-1).sum(-1)
    return x
...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0,
    ...
    note: We did not add the filter related to redundant, because the paper did not provide the parameter
          adjustment experience of multi-dimensional cosine similarity comparison
    """
    # forward
    outputs = model(x)[0]
    # adapt
